...
const DB = {
  TX:        "casa-transacoes",
  CUSTOS:    "casa-custos",
  CATS_FIN:  "casa-categorias-fin",
  LISTA:     "casa-lista-items",
  CATS_LISTA:"casa-categorias-lista",
};
...
export default function Root() {
  ...
  if (!ready || !catsFin || !transactions || !costs || listaItems === null || !catsLista) {
    return (
...
function HomePage({onNavigate,transactions,costs,listaItems}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0f2557 0%,#1e3a8a 40%,#1d4ed8 75%,#0ea5e9 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 20px",fontFamily:"'DM Sans',sans-serif",position:"relative",overflow:"hidden"}}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Mono:wght@400&family=DM+Sans:wght@400;500;600;700&display=swap');
        *{box-sizing:border-box;margin:0;padding:0}
        .hcard{transition:transform .25s ease,box-shadow .25s ease!important}
        .hcard:hover{transform:translateY(-8px)!important;box-shadow:0 28px 64px rgba(0,0,0,.4)!important}
        .hcard:hover .arr{transform:translateX(8px)!important}
        .arr{transition:transform .2s ease}
        @keyframes fadeUp{from{opacity:0;transform:translateY(28px)}to{opacity:1;transform:translateY(0)}}
        @keyframes pulse{0%,100%{opacity:.12}50%{opacity:.22}}
      `}</style>
      {[{s:380,t:-100,l:-100,d:0},{s:220,b:-80,r:-60,d:1},{s:140,t:60,r:180,d:2}].map((b,i)=>(
        <div key={i} style={{position:"absolute",width:b.s,height:b.s,borderRadius:"50%",background:"white",opacity:.12,top:b.t,bottom:b.b,left:b.l,right:b.r,animation:`pulse ${3+i}s ${b.d}s ease-in-out infinite`,pointerEvents:"none"}}/>
      ))}
      <div style={{textAlign:"center",marginBottom:52,animation:"fadeUp .6s ease both"}}>
        ...
        <h1 style={{fontFamily:"'DM Sans',sans-serif",fontSize:36,fontWeight:800,color:"white",lineHeight:1.1,letterSpacing:.5,marginBottom:8}}>Nossa Casinha</h1>
        <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,color:"rgba(255,255,255,.5)"}}>Selecione o que deseja acessar</p>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:20,width:"100%",maxWidth:640,animation:"fadeUp .6s .12s ease both",opacity:0,animationFillMode:"forwards"}}>
        <button className="hcard" onClick={()=>onNavigate("financeiro")} style={{background:"rgba(255,255,255,.11)",backdropFilter:"blur(20px)",border:"1.5px solid rgba(255,255,255,.2)",borderRadius:22,padding:"28px 26px",cursor:"pointer",textAlign:"left",boxShadow:"0 8px 32px rgba(0,0,0,.2)",position:"relative",overflow:"hidden"}}>
          ...
          <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"rgba(255,255,255,.55)",marginBottom:20,lineHeight:1.5}}>Controle de economias, custos do financiamento e pagamentos da obra</p>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"rgba(255,255,255,.65)",fontWeight:500}}>Acessar dashboard</span><span className="arr" style={{fontSize:20,color:"rgba(255,255,255,.65)"}}>→</span></div>
        </button>
        <button className="hcard" onClick={()=>onNavigate("lista")} style={{background:"rgba(255,255,255,.11)",backdropFilter:"blur(20px)",border:"1.5px solid rgba(255,255,255,.2)",borderRadius:22,padding:"28px 26px",cursor:"pointer",textAlign:"left",boxShadow:"0 8px 32px rgba(0,0,0,.2)",position:"relative",overflow:"hidden"}}>
          ...
          <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"rgba(255,255,255,.55)",marginBottom:20,lineHeight:1.5}}>Móveis, eletrodomésticos e tudo que vai fazer parte do novo lar</p>
          <div style={{background:"rgba(255,255,255,.09)",borderRadius:10,padding:"12px 14px",marginBottom:18}}>
            {listaItems.length===0
              ?<div style={{textAlign:"center",padding:"4px 0"}}><span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(255,255,255,.35)"}}>Nenhum item cadastrado ainda</span></div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"rgba(255,255,255,.65)",fontWeight:500}}>Acessar lista</span><span className="arr" style={{fontSize:20,color:"rgba(255,255,255,.65)"}}>→</span></div>
        </button>
      </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#eef3fb",color:"#1a3050",fontFamily:"'DM Sans',sans-serif"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#93b8e0;border-radius:2px}
        ...
        .tbtn:hover{background:rgba(37,99,235,.06)!important}.rh:hover{background:rgba(37,99,235,.04)!important}
        .bpri:hover{filter:brightness(.9)}.bgh:hover{background:rgba(37,99,235,.08)!important}
        .crow:hover .ca{opacity:1!important}.sw:hover{transform:scale(1.18)}.hb:hover{background:rgba(255,255,255,.25)!important}
        ...
        <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(10,20,50,.45)",display:"flex",alignItems:"center",justifyContent:"center",animation:"fIn .2s ease",padding:16}} onClick={e=>e.target===e.currentTarget&&setPayModal({open:false,person:null})}>
          <div style={{background:"#fff",borderRadius:14,width:"min(96vw,500px)",maxHeight:"85vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.22)"}}>
            <div style={{padding:"16px 20px 12px",borderBottom:"1px solid #dae6f5",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,background:"#fff"}}>
              <div><div style={{fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:3,color:payModal.person==="moises"?"#2563eb":"#7c3aed",marginBottom:4}}>{payModal.person==="moises"?"💙 PAGAMENTO MOISÉS":"💜 PAGAMENTO NORRARA"}</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:19,color:"#1a3050"}}>Registrar Pagamento de Custo</div></div>
                    ...
                    return(<div key={c.id} onClick={()=>selectPayCost(c.id)} style={{padding:"9px 11px",borderRadius:8,border:`1.5px solid ${sel?(cat?.color||"#2563eb"):"#dae6f5"}`,background:sel?(cat?.bg||"#eff6ff"):"#f8fbff",cursor:"pointer",transition:"all .15s"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          ...
                          <div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"#1a3050"}}>{c.desc}</div><div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"#9ab8d8"}}>{cat?.icon} {cat?.label}{pp>0?` · ${pp.toFixed(0)}% pago`:""}</div></div>
                        </div>
            ...
            {payForm.costId&&(
              <div style={{padding:"12px 20px 0"}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:10}}>
                  ...
                  <div style={{paddingTop:20}}>{parseFloat(payForm.value)>0&&<div style={{background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:8,padding:"8px 12px",textAlign:"center"}}><div style={{fontFamily:"'DM Mono',monospace",fontSize:9,color:"#0284c7",letterSpacing:2,marginBottom:3}}>EQUIVALE A</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:20,color:"#0284c7"}}>{(()=>{const c=costs.find(x=>x.id===payForm.costId);return c&&c.amount>0?(parseFloat(payForm.value)/c.amount*100).toFixed(1)+"%":"-";})()}</div></div>}</div>
                </div>
      ...
      <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 28px",background:"linear-gradient(135deg,#1e4d9b,#2563eb,#3b82f6)"}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <button className="hb" onClick={onHome} style={{background:"rgba(255,255,255,.15)",border:"1px solid rgba(255,255,255,.25)",borderRadius:10,padding:"8px 14px",color:"white",fontFamily:"'DM Sans',sans-serif",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6,transition:"background .2s",whiteSpace:"nowrap"}}>🏡 Home</button>
          <div><div style={{fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:4,color:"rgba(255,255,255,.55)",marginBottom:4}}>NOSSA CASINHA — MOISÉS E NORRARA</div><h1 style={{fontFamily:"'DM Sans',sans-serif",fontSize:22,fontWeight:800,color:"#fff",letterSpacing:.5}}>Planejamento Financeiro</h1></div>
        </div>
        <div style={{textAlign:"right"}}><span style={{fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:3,color:"rgba(255,255,255,.55)",display:"block",marginBottom:4}}>PROGRESSO</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:28,fontWeight:800,color:"#fff",display:"block",lineHeight:1}}>{progressSaved.toFixed(1)}%</span></div>
      </header>
      <nav style={{display:"flex",padding:"0 28px",background:"#fff",borderBottom:"1px solid #dae6f5",boxShadow:"0 1px 4px rgba(30,77,155,.06)",overflowX:"auto"}}>
        {TABS.map(t=><button key={t.id} className="tbtn" onClick={()=>setTab(t.id)} style={{background:"transparent",border:"none",padding:"13px 18px",color:tab===t.id?"#2563eb":"#7aa0c8",fontFamily:"'DM Sans',sans-serif",fontSize:13,cursor:"pointer",borderBottom:`2px solid ${tab===t.id?"#2563eb":"transparent"}`,transition:"all .2s",whiteSpace:"nowrap"}}>{t.l}</button>)}
      </nav>
      <main style={{padding:"20px 28px 40px",maxWidth:860,margin:"0 auto"}}>
        {tab==="dashboard"&&<div style={{animation:"sIn .3s ease"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:14,marginBottom:14}}>
            {[{l:"TOTAL GUARDADO",v:fmt(totalSaved),s:`em ${transactions.filter(t=>isEntrada(t.type)).length} depósitos`,a:"#2563eb",i:"🏦"},{l:"CUSTOS RESTANTES",v:fmt(totalCosts),s:`${costs.filter(c=>!isFullyPaid(c)).length} custo(s) em aberto`,a:"#6366f1",i:"📋"},{l:"FALTA GUARDAR",v:fmt(remaining),s:remaining<=0?"Meta atingida! 🎉":`${(100-progress).toFixed(1)}% do objetivo`,a:remaining<=0?"#16a34a":"#0284c7",i:remaining<=0?"✅":"🎯"}].map((k,i)=>(
              <div key={i} style={{background:"#fff",border:"1px solid #dae6f5",borderRadius:10,padding:"16px 18px",boxShadow:"0 1px 6px rgba(30,77,155,.06)",borderTop:`3px solid ${k.a}`,position:"relative",overflow:"hidden"}}>
                ...
                <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:24,color:k.a,fontWeight:800,margin:"6px 0 3px",lineHeight:1}}>{k.v}</div>
          ...
          {[{l:"💙 META GUARDADO",s:"quanto do total já foi guardado",pct:progressSaved,lft:`${fmt(totalSaved)} guardado`,rgt:`meta: ${fmt(totalGross)}`,c:"#2563eb",g:"linear-gradient(90deg,#1e4d9b,#2563eb,#60a5fa)"},{l:"✅ CUSTOS PAGOS",s:"quanto do total de custos já foi quitado",pct:progress,lft:`${fmt(totalPaidOut)} pago`,rgt:`total: ${fmt(totalGross)}`,c:"#059669",g:"linear-gradient(90deg,#065f46,#059669,#34d399)"}].map((pb,i)=>(
            <div key={i} style={{background:"#fff",border:"1px solid #dae6f5",borderRadius:10,padding:"16px 18px",marginBottom:14,boxShadow:"0 1px 6px rgba(30,77,155,.06)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:5}}><div><span style={{fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:3,color:"#7aa0c8",textTransform:"uppercase"}}>{pb.l}</span><div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"#9ab8d8",marginTop:1}}>{pb.s}</div></div><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:22,color:pb.c,fontWeight:800}}>{pb.pct.toFixed(1)}%</span></div>
          ...
          <div style={{background:"#fff",border:"1px solid #dae6f5",borderRadius:10,padding:"16px 18px",marginBottom:14,boxShadow:"0 1px 6px rgba(30,77,155,.06)"}}>
            ...
            <div>{[...transactions].sort((a,b)=>new Date(b.date)-new Date(a.date)).slice(0,5).map(t=>(
              <div key={t.id} className="rh" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 8px",borderRadius:6,transition:"background .15s",borderBottom:"1px solid #d0e3f5"}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:15}}>{isEntrada(t.type)?"⬆":isPayment(t.type)?"💳":"⬇"}</span><div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"#1a3050"}}>{t.desc}</div><div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"#9ab8d8",marginTop:1}}>{fmtDate(t.date)} · {labelOf(t.type)}</div></div></div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {catsFin.map(cat=>{
                ...
                return(<div key={cat.id} style={{background:cat.bg,border:`1px solid ${cat.border}`,borderRadius:8,padding:"10px 12px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:7}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:15}}>{cat.icon}</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:500,color:cat.color}}>{cat.label}</span></div><span style={{fontFamily:"'DM Mono',monospace",fontSize:13,color:cat.color}}>{fmt(tot-paid)}</span></div>
                  ...
                  {items.map((item,idx)=>{const fp=isFullyPaid(item);return(<div key={item.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:idx<items.length-1?"1px solid rgba(255,255,255,.7)":"none",opacity:fp?.6:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:5}}>{fp&&<span style={{fontSize:12}}>✅</span>}<span style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#4a6080",textDecoration:fp?"line-through":"none"}}>· {item.desc}</span></div>
        ...
        {tab==="lancamentos"&&<div style={{animation:"sIn .3s ease"}}>
          <div style={{background:"#fff",border:"1px solid #dae6f5",borderRadius:10,padding:"16px 18px",marginBottom:14,boxShadow:"0 1px 6px rgba(30,77,155,.06)"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12}}>
              ...
              {isPayment(txForm.type)?<div style={{display:"flex",alignItems:"flex-end",gridColumn:"span 3"}}><div style={{flex:1,background:"#fffbeb",border:"1px solid #fde68a",borderRadius:8,padding:"10px 13px"}}><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"#92400e"}}>💳 Clique em <strong>"Selecionar Custo"</strong> para registrar o pagamento.</span></div></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
            {[{p:"Moisés",c:"#2563eb",bg:"#eff6ff",br:"#bfdbfe",e:"💙"},{p:"Norrara",c:"#7c3aed",bg:"#faf5ff",br:"#e9d5ff",e:"💜"}].map(x=>(
              <div key={x.p} style={{background:x.bg,border:`1px solid ${x.br}`,borderRadius:10,padding:"12px 16px",borderTop:`3px solid ${x.c}`}}>
                ...
                <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:22,fontWeight:800,color:x.c}}>{fmt(pSaved(x.p))}</div>
          ...
          <div style={{background:"#fff",border:"1px solid #dae6f5",borderRadius:10,padding:"16px 18px",boxShadow:"0 1px 6px rgba(30,77,155,.06)"}}>
            ...
            <div>{[...transactions].sort((a,b)=>new Date(b.date)-new Date(a.date)).map(t=>{
              ...
              return(<div key={t.id} className="rh" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 8px",borderRadius:6,transition:"background .15s",borderBottom:"1px solid #d0e3f5"}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:30,height:30,borderRadius:"50%",background:`${col}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:col,flexShrink:0}}>{en?"↑":pay?"💳":"↓"}</div><div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"#1a3050"}}>{t.desc}</div><div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"#9ab8d8",marginTop:1}}>{fmtDate(t.date)} · {labelOf(t.type)}</div></div></div>
        ...
        {tab==="custos"&&<div style={{animation:"sIn .3s ease"}}>
          <div style={{background:"#fff",border:"1px solid #dae6f5",borderRadius:10,padding:"16px 18px",marginBottom:14,boxShadow:"0 1px 6px rgba(30,77,155,.06)"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12}}>
              <div style={{gridColumn:"1 / -1"}}><label style={F.il}>Categoria</label><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{catsFin.map(cat=><button key={cat.id} onClick={()=>setCostForm(p=>({...p,category:cat.id}))} style={{padding:"6px 12px",borderRadius:20,border:`1.5px solid ${costForm.category===cat.id?cat.color:"#ccdcf0"}`,background:costForm.category===cat.id?cat.bg:"transparent",color:costForm.category===cat.id?cat.color:"#7aa0c8",fontFamily:"'DM Sans',sans-serif",fontSize:12,cursor:"pointer",transition:"all .15s",display:"flex",alignItems:"center",gap:4}}><span>{cat.icon}</span>{cat.label}</button>)}</div></div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {catsFin.map(cat=>{const isDef=!!DEFAULT_CATS_FIN.find(d=>d.id===cat.id);return(
                <div key={cat.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"7px 9px",borderRadius:6,background:cat.bg,border:`1.5px solid ${editCat===cat.id?cat.color:cat.border}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:17}}>{cat.icon}</span><div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:cat.color,fontWeight:500}}>{cat.label}</div><div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"#9ab8d8"}}>{costs.filter(c=>c.category===cat.id).length} item(ns)</div></div></div>
          ...
          {costs.length>0&&<div style={{background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"1px solid #93c5fd",borderRadius:10,padding:"16px 18px",marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:3,color:"#7aa0c8",textTransform:"uppercase",marginBottom:3}}>CUSTOS TOTAIS ATIVOS</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:28,fontWeight:800,color:"#1e4d9b"}}>{fmt(totalCosts)}</div></div>
              <div style={{textAlign:"right"}}><div style={{fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:3,color:"#7aa0c8",textTransform:"uppercase",marginBottom:3}}>FALTA GUARDAR</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:22,fontWeight:800,color:remaining<=0?"#16a34a":"#0284c7"}}>{fmt(remaining)}</div></div>
            </div>
          </div>}
          {catsFin.map(cat=>{
            ...
            return(<div key={cat.id} style={{background:"#fff",border:"1px solid #dae6f5",borderRadius:10,padding:"16px 18px",marginBottom:14,boxShadow:"0 1px 6px rgba(30,77,155,.06)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:items.length>0?10:0}}>
                <div style={{display:"flex",alignItems:"center",gap:7}}><span style={{fontSize:17}}>{cat.icon}</span><span style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:500,color:cat.color}}>{cat.label}</span><span style={{fontFamily:"'DM Mono',monospace",fontSize:9,background:cat.bg,color:cat.color,border:`1px solid ${cat.border}`,borderRadius:10,padding:"2px 7px"}}>{items.length} item(ns)</span></div>
                <span style={{fontFamily:"'DM Mono',monospace",fontSize:14,color:cat.color}}>{fmt(subtotal)}</span>
              </div>
              {items.length===0?<div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#9ab8d8",paddingTop:4}}>Selecione esta categoria para adicionar itens.</div>:
              <div>{items.map((c,i)=>{
                const fp=isFullyPaid(c),pp=paidPct(c),rem=Math.max(c.amount-(c.paidAmount||0),0);
                return(<div key={c.id} className="crow rh" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 8px",borderRadius:6,transition:"background .15s",borderBottom:"1px solid #d0e3f5",opacity:fp?.65:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,flex:1}}>
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"#b0c8e0",minWidth:16}}>{String(i+1).padStart(2,"0")}</div>
                    <div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:5}}>{fp&&<span>✅</span>}<span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"#1a3050",textDecoration:fp?"line-through":"none"}}>{c.desc}</span></div><div style={{display:"flex",alignItems:"center",gap:7,marginTop:2}}><span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"#9ab8d8"}}>{pp.toFixed(0)}% pago</span>{pp>0&&pp<100&&<div style={{flex:1,maxWidth:70,background:"#dae6f5",borderRadius:2,height:3}}><div style={{width:`${pp}%`,height:"100%",background:cat.color,borderRadius:2}}/></div>}</div></div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                    ...
                    <div className="ca" style={{display:"flex",gap:5,opacity:0,transition:"opacity .2s"}}><button className="bgh" onClick={()=>startEditCost(c)} style={{...F.bg,padding:"3px 7px",fontSize:11,color:"#6366f1"}}>✎</button><button className="bgh" onClick={()=>removeCost(c.id)} style={{...F.bg,padding:"3px 7px",fontSize:11,color:"#dc2626",borderColor:"#fecaca"}}>✕</button></div>
                  </div>
                </div>);
              })}<div style={{borderTop:"1px solid #dae6f5",marginTop:5,paddingTop:8,display:"flex",justifyContent:"flex-end",gap:14}}><span style={{fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:3,color:"#7aa0c8",textTransform:"uppercase"}}>SUBTOTAL RESTANTE</span><span style={{fontFamily:"'DM Mono',monospace",fontSize:13,color:cat.color}}>{fmt(subtotal)}</span></div></div>}
            </div>);
          })}
        </div>}
      </main>
...
function ListaApp({listaItems,setListaItems,catsLista,setCatsLista,onHome}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#eff6ff,#dbeafe 40%,#e0f2fe 100%)",fontFamily:"'DM Sans',sans-serif"}}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Mono:wght@400&family=DM+Sans:wght@400;600;700&display=swap');
        *{box-sizing:border-box;margin:0;padding:0}
        ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#93c5fd;border-radius:2px}
        input,select{outline:none}
        input:focus,select:focus{border-color:#1d4ed8!important;box-shadow:0 0 0 3px rgba(29,78,216,.1)}
        .lhb:hover{background:rgba(255,255,255,.25)!important}
        .li:hover{box-shadow:0 4px 16px rgba(30,58,138,.13)!important}
        @keyframes sIn{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:translateY(0)}}
        @keyframes fIn{from{opacity:0}to{opacity:1}}
      `}</style>
      {toast&&<div style={{position:"fixed",top:24,right:24,zIndex:2000,background:toast.ok?"#eff8ff":"#fff0f0",border:`1px solid ${toast.ok?"#93c5fd":"#fca5a5"}`,color:toast.ok?"#1d4ed8":"#dc2626",padding:"10px 18px",borderRadius:8,fontFamily:"'DM Sans',sans-serif",fontSize:13,animation:"sIn .2s ease",boxShadow:"0 4px 16px rgba(0,0,0,.1)"}}>{toast.msg}</div>}
        ...
        <div style={{maxWidth:700,margin:"0 auto",position:"relative"}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:18}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <button className="lhb" onClick={onHome} style={{background:"rgba(255,255,255,.15)",border:"1px solid rgba(255,255,255,.25)",borderRadius:10,padding:"8px 13px",color:"white",fontFamily:"'DM Sans',sans-serif",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6,transition:"background .2s",whiteSpace:"nowrap"}}>🏡 Home</button>
      ...
      <div style={{maxWidth:700,margin:"18px auto 0",padding:"0 14px 60px",animation:"sIn .3s ease"}}>
        <div style={{background:"white",borderRadius:14,boxShadow:"0 4px 20px rgba(30,58,138,.08)",marginBottom:14,overflow:"hidden",border:"1px solid #e0eaff"}}>
          <button onClick={()=>{setShowForm(!showForm);setEditId(null);setForm({descricao:"",valor:"",link:"",categoria:"Sala",prioridade:"média"});}} style={{width:"100%",padding:"14px 18px",display:"flex",alignItems:"center",gap:11,background:"none",border:"none",cursor:"pointer",color:"#1d4ed8",fontWeight:700,fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>
        ...
        {listaItems.length>0&&<div style={{marginBottom:12,display:"flex",flexDirection:"column",gap:8}}>
          ...
          <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
            {[{k:"todos",l:`Todos (${listaItems.length})`},{k:"pendentes",l:`Pendentes (${listaItems.filter(i=>!i.comprado).length})`},{k:"comprados",l:`Comprados (${listaItems.filter(i=>i.comprado).length})`}].map(f=>(
              <button key={f.k} onClick={()=>setFilter(f.k)} style={{padding:"5px 13px",borderRadius:99,border:"2px solid",borderColor:filter===f.k?"#1d4ed8":"#dbeafe",background:filter===f.k?"#1d4ed8":"white",color:filter===f.k?"white":"#1d4ed8",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:12,cursor:"pointer",transition:"all .15s"}}>{f.l}</button>
            ))}
            <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{marginLeft:"auto",padding:"5px 11px",borderRadius:99,border:"2px solid #dbeafe",background:"white",color:"#1d4ed8",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:12,cursor:"pointer",outline:"none"}}>
        ...
        {listaItems.length>0&&<div style={{marginTop:18,background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)",borderRadius:12,padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"0 4px 20px rgba(30,58,138,.25)"}}>
          <div><p style={{color:"rgba(255,255,255,.6)",fontSize:10,fontFamily:"'DM Mono',monospace",letterSpacing:2,margin:"0 0 3px",textTransform:"uppercase"}}>TOTAL GERAL DA LISTA</p><p style={{color:"white",fontSize:26,fontFamily:"'DM Sans',sans-serif",fontWeight:700,margin:0}}>{fmt(total)}</p></div>
